return "return {[\"background\"] = \"gui_brewerbg.png\", [\"title\"] = \"Making Potions\", [\"owner\"] = \"An Elven Potion Brewer\"," ..
	"[\"text\"] = \"" ..
	"\\\nPotion brewing is a hard and delicate process the require time to master and finding the rarest of ingredients."..
	"\\\nBut the result of which might be very rewarding.."..
	"\\\nBrews are capable of making the drinker perform extraordinary feats, both physical and metal, after he drinks them."..
	"\\\n"..
	"\\\nPotion Brewing Process"..
	"\\\n--------------------------------------------------------------------" ..
	"\\\nTo start off, you first need a cauldron and a potion brewer."..
	"\\\nI will not elaborate overly on their recipies, because you will be able to find them in your Craft Book, on pages 17 and 19."..
	"\\\n"..
	"\\\nAfter which, you need a Book of Potions. You can get that by putting a Cooking Book beside a Potion Brewer."..
	"\\\n"..
	"\\\nPlace the cauldron and fill the cauldron with water by rightclicking it with a bucket of water."..
	"\\\nCraft a glass bottle and punch it on cauldron to get a glass bottle filled with water."..
	"\\\n"..
	"\\\nThen proceed by combining the glass bottle with various ingredients as stated in your Book of Potions."..
	"\\\n"..
	"\\\nYou first need to brew a base potion, which can be found on the first page of the Book of Potions; this base potion will serve as the base for several other potions, but by itself, has no effect."..
	"\\\nAfter which, simply combine the base potion with the respective ingredients to get the potions you want."..
	"\\\n"..
	"\\\nA potion can be crafted together with an arrow, which will produce a potion bound to an arrow. You can fire these arrows with a bow, and whoever is hit by the arrow will be inflicted with the effects of the potion, be they good or bad."..
	"\\\n"..
	"\\\nFinding Ingredients"..
	"\\\n--------------------------------------------------------------------" ..
	"\\\nPotions uses some of the rarest ingredients, as previously mentioned, that are hard to find."..
	"\\\nThese ingreidents are scattered all over Middle Earth, both above and below the surface."..
	"\\\nAs I mentioned, potion brewing is a hard and delicate process, so I will not provide you with the direct answers."..
	"\\\nYou will have to explore the methods and find the answers yourself."..
	"\\\nIf you look carefully, you may be able to find other guidebooks or handbooks which can provide you with more assistance in locating the whereabouts of these ingredients.\"}"
