...
if minetest.get_modpath("olvar") ~= nil then
	overwrite("olvar:barley", 4)
	overwrite("olvar:berries", 2)
	overwrite("olvar:blue_mushroom", 3)
	overwrite("olvar:brown_mushroom", 4)
	overwrite("olvar:cabbage", 4)
	overwrite("olvar:corn", 6)
	overwrite("olvar:green_mushroom", 3)
	overwrite("olvar:melon", 4)
	overwrite("olvar:potato", 1)
	overwrite("olvar:potato_cooked", 8)
	overwrite("olvar:red_mushroom", 4)
	overwrite("olvar:tomatoes", 2)
	overwrite("olvar:tomatoes_cooked", 4)
	overwrite("olvar:tomato_soup", 8)
	overwrite("olvar:turnips", 1)
	overwrite("olvar:turnips_cooked", 8)
	overwrite("olvar:salad", 6)
	overwrite("olvar:mushroom_soup", 8)
end

if minetest.get_modpath("olvar") ~= nil then
	overwrite("olvar:plum", 2)
	overwrite("olvar:yavannamirefruit", 10)
	overwrite("olvar:honey", 2)
end

if minetest.get_modpath("kelvar") ~= nil then
	overwrite("kelvar:meat", 4)
end
