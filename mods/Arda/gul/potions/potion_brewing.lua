...
function gul.register_recipe_type(typename, origdata)
	...
	data.recipes = {}
	gul.potion_recipes[typename] = data
end
...
local function register_recipe(typename, data)
	...
	gul.potion_recipes[typename].recipes[index] = recipe
end

function gul.register_recipe(typename, data)
	minetest.after(0.01, register_recipe, typename, data) -- Handle aliases
end

function gul.get_potion_recipe(typename, items)
	...
	local recipe = gul.potion_recipes[typename].recipes[index]
	if recipe then
		local new_input = {}
		for i, stack in ipairs(items) do
...
gul.register_recipe_type("potionbrew", {
	description = "Potion Brewing",
	input_size = 2,
})

function gul.register_potionbrew_recipe(data)
...
local formspec =
	...
	"image[4,2;1,1;gul_bubble_off.png]"..
     "image[3,2;1,1;gul_arrow.png]"..
     "image[5,2;1,1;gul_arrow.png]"..
     "label[3.2,3.2;Fuel:]"..
...
minetest.register_node("gul:potion_brewer", {
	description = machine_name,
	drawtype = "plantlike",
	visual_scale = 1.0,
	tiles = {"gul_potion_brewer.png"},
	inventory_image = {"gul_potion_brewer.png"},
	wield_image = {"gul_potion_brewer.png"},
	paramtype = "light",
...
minetest.register_node("gul:potion_brewer_active", {
	description = machine_name,
	drawtype = "plantlike",
	visual_scale = 1.0,
	tiles = {"gul_potion_brewer_active.png"},
     inventory_image = {"gul_potion_brewer.png"},
	wield_image = {"gul_potion_brewer.png"},
	paramtype = "light",
	light_source = 8,
...
minetest.register_abm({
	nodenames = {"gul:potion_brewer", "gul:potion_brewer_active"},
	interval = 1,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local result = gul.get_potion_recipe("potionbrew", inv:get_list("src"))
		...
		if meta:get_float("fuel_time") < meta:get_float("fuel_totaltime") then
			...
			gul.swap_node(pos, "gul:potion_brewer_active")
			meta:set_string("formspec",
					"size[8,9]"..
					"label[0,0;"..machine_name.."]"..
					"image[4,2;1,1;gul_bubble_off.png^[lowpart:"..
					(percent)..":gul_bubble.png]"..
     				"image[3,2;1,1;gul_arrow.png]"..
     				"image[5,2;1,1;gul_arrow.png]"..
					"label[3.2,3.2;Fuel:]"..
					"list[current_name;fuel;4,3;1,1;]"..
		...
		local recipe = gul.get_potion_recipe("potionbrew", inv:get_list("src"))

		if not recipe then
			if was_active then
				meta:set_string("infotext", ("%s is empty"):format(machine_name))
				gul.swap_node(pos, "gul:potion_brewer")
				meta:set_string("formspec", formspec)
			end
			return
		end
