...
function gul.register_recipe_type(typename, origdata)
	...
	data.recipes = {}
	gul.brew_recipes[typename] = data
end
...
local function register_recipe(typename, data)
	...
	gul.brew_recipes[typename].recipes[index] = recipe
end
...
function gul.get_brew_recipe(typename, items)
	...
	local recipe = gul.brew_recipes[typename].recipes[index]
	if recipe then
		local new_input = {}
		for i, stack in ipairs(items) do
...
gul.register_recipe_type("brew", {
	description = "Brewing",
	input_size = 2,
})

function gul.register_brew_recipe(data)
...
minetest.register_node("gul:brewer", {
	description = machine_name,
	drawtype = "nodebox",
	tiles = {"default_wood.png"},
	node_box = {
        type = "fixed",
            fixed = gul.make_pipe( { {f=0.9,h1=-0.2,h2=0.2,b=0}, {f=0.75,h1=-0.50,h2=-0.35,b=0}, {f=0.75,h1=0.35,h2=0.5,b=0},
...
minetest.register_node("gul:brewer_active", {
	description = machine_name,
	drawtype = "nodebox",
     tiles = {"default_wood.png"},
	node_box = {
...
minetest.register_abm({
	nodenames = {"gul:brewer", "gul:brewer_active"},
	interval = 1,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local result = gul.get_brew_recipe("brew", inv:get_list("src"))
		...
		if meta:get_float("fuel_time") < meta:get_float("fuel_totaltime") then
			...
			gul.swap_node(pos, "gul:brewer_active")
		...
		local recipe = gul.get_brew_recipe("brew", inv:get_list("src"))

		if not recipe then
			if was_active then
				meta:set_string("infotext", ("%s is empty"):format(machine_name))
				gul.swap_node(pos, "gul:brewer")
				meta:set_string("formspec", formspec)
			end
			return
		end
