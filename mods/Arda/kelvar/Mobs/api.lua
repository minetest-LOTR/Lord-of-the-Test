...
kelvar.spawning_kelvar = {}
function kelvar:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, spawn_func)
	kelvar.spawning_kelvar[name] = true	
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if not kelvar.spawning_kelvar[name] then
				return
			end
			...
			if minetest.setting_getbool("display_mob_spawn") then
				minetest.chat_send_all("[kelvar] Add "..name.." at "..minetest.pos_to_string(pos))
			end
...
function kelvar:register_arrow(name, def)
	minetest.register_entity(name, {
		physical = false,
		visual = def.visual,
		visual_size = def.visual_size,
