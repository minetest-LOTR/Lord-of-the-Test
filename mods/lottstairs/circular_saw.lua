...
circular_saw.cost_in_microblocks = {
	--mat, cube, step, halfstair, sphere
	1, 1, 2, 3, 4,
	--slab, outerstair, stair, innerstair, full
	4, 5, 6, 7, 8,
...
function circular_saw:get_output_inv(modname, material, amount, max)
	...
	for i = 1, #circular_saw.names do
		...
		local nodename = ""
		if t[1] == "full" then
			local node_name = node_name:gsub("lottstairs:", "", 1)
...
function circular_saw:update_inventory(pos, amount)
	...
	if stack:is_empty() then
		...
		local modname  = "lottstairs"
		--local material = node_name:match(":(.*)")
		local material = node_name:gsub("lottstairs:", "", 1)
...
function circular_saw.allow_metadata_inventory_put(
	...
	if listname == "input" and minetest.registered_nodes["lottstairs:"..materialname.."_cube"] then
		...
		if not inv:is_empty("micro") then
			local microstackname = inv:get_stack("micro", 1):get_name():gsub("lottstairs:", "", 1)
	...
	if listname == "micro" and materialname:match("[A-Za-z0-9%.%%%+%-]+_cube") and stackname:match("lottstairs:"..materialname) then
		...
		if not inv:is_empty("input") then
			...
			local cutstackname = cutstackname:gsub("lottstairs:", "", 1)
...
function circular_saw.on_construct(pos)
	local meta = minetest.get_meta(pos)
	local fancy_inv = ""
	meta:set_string("formspec", "size[8,7]"..fancy_inv..
			
			"list[current_name;input;0,0;1,1;]" ..
			"image[0.05,0.1;0.8,0.8;icon_input.png]" ..
			"image[1.05,0.1;0.8,0.8;gui_furnace_arrow_bg.png^[transformFYR90]" ..
			
			"list[current_name;micro;0,1;1,1;]" ..
			"image[1.06,1.1;0.8,0.8;gui_furnace_arrow_bg.png^[transformFYR90]" ..
			"image[0.06,1.1;0.8,0.8;icon_cube.png]" ..
			
			"list[detached:creative_trash;main;0,2;1,1;]"..
			"image[0.06,2.1;0.8,0.8;creative_trash_icon.png]" ..
			--"list[current_name;recycle;1,2;1,1;]" ..
			
			"field[7.3,1.5;1,1;max_offered;" .."Max".. ":;${max_offered}]" ..
			"button[7,2.1;1,1;Set;" .."Set".. "]" ..
			
			"list[current_name;output;2,0;5,3;]" ..
	...
	inv:set_size("output", 5*3) -- 6x8 versions of stair-parts of material x.

	circular_saw:reset(pos)
end

minetest.register_node("lottstairs:circular_saw",  {
	...
	tiles = {"lottstairs_circular_saw_top.png",
		"lottstairs_circular_saw_bottom.png",
		"lottstairs_circular_saw_side.png"},
	paramtype = "light",
